...
async function editorAcceptPubReq(requestId = 0){
	try {
    const result = await axios.get(import.meta.env.VITE_URL_API + "/request/" + requestId + "/valid");
		return result.data;
  } catch (err) {
    return err;
    return err.response.data.error;
...
async function editorRefusePubReq(requestId = 0){
	try {
    const result = await axios.get(import.meta.env.VITE_URL_API + "/request/" + requestId + "/refuse");
		return result.data;
  } catch (err) {
    return err;
    return err.response.data.error;
